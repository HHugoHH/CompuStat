...
# want to estimate theta = (mu, sigma2) unknown. 
...
plot(prior.mean, col="darkblue", xlim=c(0,7), lwd="2", main="Prior for mean", ylab="density")
...
data <- matrix(c(iris$Sepal.Width,iris$Petal.Width,iris$Sepal.Length,iris$Petal.Length), ncol=4)
...
#w0+w1x1+w2x2+w3x3 = x4

objdens(data, c(2,3,2,3))
...
thinning <- 2 # meaning we'll keep 75% of observations to reduce autocorrelation
